...
class College():
    ...
    def get_all(self, page_num: int = None, item_per_page: int = None, paginate: bool = True) -> list:
        ...
        # query = f'''
        #     SELECT college.code, college.name,college.logo ,COUNT(*) AS courses, enrolled.student as enrolled
        #     FROM college
        #     JOIN course
        ...
        # '''
        query = f'''
            SELECT college.code, college.name,college.logo 
            FROM college
        '''
    ...
    @staticmethod
    def get_image_url(code: str = None) -> str:
        query = f'''
            SELECT logo
            FROM college
            WHERE code = '{code}'
